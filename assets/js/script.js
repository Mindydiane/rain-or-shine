...
function formSubmitHandler(event) {
  ...
  var currentCity = cityInputField.value.trim();
...
clearHistory.addEventListener("click", function () {
  var accept = confirm(
    "Search History will be cleared! Do you want to proceed?"
  );
  if (accept === true) {
    localStorage.clear();
        ...
        weatherResponse.json().then(function (weatherResponse) {
          ...
          fetch(
            ...
            .then(function (response) {
              ...
              var temperatureValue = Math.round(
              ...
              currentWeatherCond.textContent =
                currentWeatherCondition.toUpperCase();
    ...
    if (forecastResponse.ok) {
      forecastResponse.json().then(function (forecastResponse) {
        ...
        for (let i = 0; i < forecastResponse.list.length; i++) {
          ...
          var certainTimeData =
            forecastResponse.list[i]["dt_txt"].split(" ")[1].split(":")[0] ==
            15;
          if (certainTimeData) {
